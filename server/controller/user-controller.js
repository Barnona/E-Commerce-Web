...
const userSignup = async (request, response) => {
    try {
        ...
        // Never return the submitted password or the password hash.
...
const forgotPassword = async (request, response) => {
    try {
        ...
        const clientUrl = (process.env.CLIENT_URLS || 'http://localhost:5173')
            .split(',')[0]
            .trim()
            .replace(/\/$/, '');
...
const saveAddress = async (request, response) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(
            id,
            { $push: { addresses: { line1, line2, city, zip, state, country } } },
            { new: true }
        );
...
const removeAddress = async (request, response) => {
    try {
        const { id, addressId } = request.body;
        const user = await User.findByIdAndUpdate(
            id,
...
const getAllUsers = async (request, response) => {
    try {
        ...
        const formattedUsers = users.map(user => {
            ...
            const dateStr = user.createdAt ? new Date(user.createdAt).toLocaleDateString('en-GB') : 'N/A';
