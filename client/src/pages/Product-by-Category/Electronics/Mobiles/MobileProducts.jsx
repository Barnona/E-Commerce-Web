...
const MobileProducts = ({ filters, apiData }) => {
  const productsToDisplay = Array.isArray(apiData) ? apiData : [];
  ...
  let filteredProducts = productsToDisplay.filter((product) => {
    ...
    const priceMatch = maxPrice === null || getFinalPrice(product) <= maxPrice;
    return ramMatch && romMatch && brandMatch && displayTypeMatch && priceMatch;
  });

  if (filters.sort === "price-asc") filteredProducts = [...filteredProducts].sort((a, b) => getFinalPrice(a) - getFinalPrice(b));
  if (filters.sort === "price-desc") filteredProducts = [...filteredProducts].sort((a, b) => getFinalPrice(b) - getFinalPrice(a));

  if (filteredProducts.length === 0) return <div className="text-center p-10 text-gray-500">No products found matching your filters.</div>;
  ...
  return (
    <div className="h-min grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 flex-1">
      {filteredProducts.map((product) => {
        ...
        return (
          <div key={productId} className="border rounded-md p-4 bg-white hover:shadow-lg transition">
              ...
              <div className="flex gap-x-8 overflow-hidden justify-center">
                {Array.isArray(product.images) && product.images.length > 0 ? (
                  <Swiper navigation modules={[Navigation]} className="mySwiper w-60 h-60">
                    {product.images.map((img, index) => <SwiperSlide key={`${productId}-${index}`}><img src={img} alt={product.name || "Product"} className="w-full h-full object-contain" /></SwiperSlide>)}
                  </Swiper>
